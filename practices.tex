...
\begin{table}[ht]
  \begin{edtable}{tabular}{p{3cm}p{2cm}p{5cm}p{5cm}}
    \toprule
    \textbf{SBOL Entity} & \textbf{Property} & \textbf{Preferred External Resource}
    & More Information \\
    \midrule
    \textbf{ComponentDefinition}  & types & BioPAX & \url{http://www.biopax.org}\\
    						   	  & roles & SO (if type is \textit{DNA} or \textit{RNA}) & \url{http://www.sequenceontology.org}   \\
    						   	  & roles & CHEBI (if type is \textit{small molecule}) & \url{https://www.ebi.ac.uk/chebi/}   \\
