...
\twozeroone{Namespaces corresponding to different versions of SBOL SHOULD NOT be semantically mixed in the same document. For example an SBOL 2.0.1 ComponentInstance SHOULD NOT refer to an SBOL 1.1 DNAComponent.  However, different versions MAY be present in the same document so long as they are semantically independent. In case multiple SBOL namespaces are present in a document, libraries SHOULD default to interpreting the most recent version of SBOL in the document. Any remaining elements in any other SBOL namespace SHOULD be treated as generic top levels and custom annotations by serialization libraries.}
