...
Namespaces for different versions of SBOL SHOULD NOT be semantically mixed in the same document. For example, an SBOL 2.0.1 \sbol{ComponentInstance} SHOULD NOT refer to an SBOL 1.1 DNAComponent. However, namespaces for different versions MAY be present in the same document so long as they are semantically independent (that is, so long as objects belonging to these namespaces do not refer to each other). When multiple SBOL namespaces are present in a document, libraries SHOULD default to interpreting as SBOL only those objects belonging to the namespace for the most recent version of SBOL in the document. Any remaining objects belonging to any other SBOL namespace SHOULD be interpreted as \sbol{GenericTopLevel}s and custom \sbol{Annotation}s.}
...
\twozeroone{\subsection{Annotating Entities with Date \& Time}
...
The PROV-O ontology (https://www.w3.org/TR/prov-o/) already defines a data model for provenance and a subset of PROV-O is adopted for SBOL as a best practice to describe these activities. PROV-O defines three core classes: \texttt{Entity}, \texttt{Activity} and \texttt{Agent}. An Agent (for example a software or a person) runs an Activity (according to a Plan) to generate one Entity from another. Although, PROV-O provides many other classes and rich set of terms. this specification describes the minimal subset of PROV-O that a provenance-aware SBOL tool must be able to handle. This subset (\ref{uml:provenance}) includes the PROV-O's Activity, Usage, Association, Agent and Plan classes. Any SBOL's Identified object implicitly act as an instance of PROV-O's Entity class and can include provenance through relationships to different Activities. Agent, Activity and Plan classes should be handled as \sbol{GenericTopLevel}, whilst Usage and Association should be nested within their parent Activities. 
...
The \sbol{startedAtTime} property is OPTIONAL and contains a dateTime value, indicating when the activity started.

\paragraph{The \sbolheading{endedAtTime} property}\label{sec:endedAtTime}
The \sbol{endedAtTime} property is OPTIONAL and contains a dateTime value, indicating when the activity ended.
...
The \sbol{hadPlan} property is OPTIONAL and contains a URI that refers to a Plan object.
...
\subsubsection{Usage}
\label{sec:Usage}
How different entities are used in an \texttt{Activity} is specified with the \texttt{Usage} class, which is linked from an \texttt{Activity} through the \texttt{qualifiedUsage} relationship. A \texttt{Usage} is then linked to an \texttt{Entity} through the \texttt{Entity}'s URI and the role of this entity is qualified with the \texttt{hadRole} property. SBOL already supports dcumenting derivation between entities using PROV-O's \sbol{wasDerivedFrom} in a lightweight provenance scheme. When this property is used together with the full provenance described here, the entity pointed at by the \sbol{wasDerivedFrom} property MUST be included in a \sbol{Usage}.
...
Bacterial strains are often derived from parent strains through modifications such as gene knockouts or mutations. \textit{Bacillus subtilis} 168 is a laboratory strain and has several advantages as a model organism in synthetic biology. \textit{B. subtilis} 168 strains are easy to transform and are not motile, facilitating the analysis of engineered cells easily. This strain was derived from the Marburg strain in the 1940s through x-radiation. The Marburg strain, on the other hand, is motile and also more difficult to transform compared to the 168 strain. In the example below, how the 168 was derived from the Marburg strain is shown.
