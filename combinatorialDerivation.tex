...
When a \sbol{Component} is derived in accordance with a \sbol{CombinatorialDerivation}, the \sbol{prov:wasDerivedFrom} property of the derived \sbol{Component} SHOULD refer to the \sbol{CombinatorialDerivation}. When multiple \sbol{Component} objects are derived in accordance with the same \sbol{CombinatorialDerivation}, they MAY be referred to by the \sbol{member} property of a \sbol{Collection}, in which case the \sbol{prov:wasDerivedFrom} property of the \sbol{Collection} SHOULD also refer to this \sbol{CombinatorialDerivation}.

If the \sbolmult{types:CD}{type} property of the template \sbol{Component} contains one or more URIs, then the \sbolmult{types:CD}{type} property of any derived \sbol{Component} SHOULD also contain those URIs. 
The same holds true for the \sbolmult{roles:CD}{role} properties of these \sbol{Component} objects.
...
Accordingly, a \sbol{CombinatorialDerivation} object can have zero or more \sbol{hasVariableComponent} properties, each of type \sbol{URI} specifying a \sbol{VariableComponent}. 
...
The \sbol{variable} properties of these \sbol{VariableComponent} objects control which \sbol{SubComponent} objects in the \sbol{template} are modified in a derived \sbol{Component}.
...
and a \sbol{prov:wasDerivedFrom} property that refers to the template \sbol{SubComponent}.
...
The \sbol{instanceOf} properties of these \sbol{SubComponent} objects in the derived \sbol{Component} MUST refer to a \sbol{Component} objects specified by a \sbol{variant}, contained within a \sbol{variantCollection}, or derived from a \sbol{variantDerivation} of the \sbol{VariableComponent}.


Finally, all derived \sbol{SubComponent} objects MUST follow the \sbol{restriction} properties of any 
\sbol{Constraint} objects that refer to their corresponding template \sbol{SubComponent}, and SHOULD have values of \sbol{role} that contain the same values as the values \sbol{role} in the template \sbol{SubComponent}.
...
As described \sbolmult{sec:hasVariableComponent}{above}, the \sbol{VariableComponent} class specifies a variable and set of values that will replace one of the \sbol{SubComponent} objects in the \sbol{template} of a \sbol{CombinatorialDerivation}.
...
The \sbol{cardinality} property is REQUIRED and has a data type of URI. This property specifies how many \sbol{SubComponent} objects SHOULD be derived from the template \sbol{SubComponent} during the derivation of a new \sbol{Component}. The URI value of this property MUST come from the URIs provided in~\ref{tbl:cardinality}.
