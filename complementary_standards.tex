...
The most basic and general type of provenance relationship can be represented using the \sbol{prov:wasDerivedFrom} property. This relationship describes derivation of an SBOL entity from another. 
Any \sbol{Identified} object may be annotated with this property. More specific provenance relationships can also be defined using PROV-O, such as \sbol{prov:wasGeneratedBy}. Generation of a new object is defined by the W3C PROV-O specification as follows:
\begin{quote}
...
\begin{table}[ht]
	\begin{edtable}{tabular}{llp{3.75in}}
		...
		\midrule
		Design  & \url{http://sbols.org/v3\#design}  & A \sbol{prov:Activity} describes the process by which a conceptual representation of an engineer's imagined and intended design for a biological system is created or derived.\\
...
A generated \texttt{Entity} is linked through a \texttt{prov:wasGeneratedBy} relationship to an \sbol{prov:Activity}, which is used to describe how different \sbol{prov:Agent}s and other entities were used. An \sbol{prov:Activity} is linked through a \sbol{prov:qualifiedUsage} to \sbol{Association}s, to describe the role of agents, and is linked through \sbol{prov:qualifiedUsage} to \sbol{prov:Usage}s to describe the role of other entities used as part of the activity. Moreover, each \sbol{prov:Activity} includes optional \sbol{prov:startedAtTime} and \sbol{prov:endedAtTime} properties. When using \sbol{prov:Activity} to capture how an entity was derived, it is expected that any additional information needed will be attached as annotations. This may include software settings or textual notes. Activities can also be linked together using the \sbol{prov:wasInformedBy} relationship to provide dependency without explicitly specifying start and end times.

\subparagraph{The \sbolheading{type} property}\label{sec:types:Activity}
The \sbolmult{types:Activity}{type} property is an OPTIONAL set of \sbol{URI}s that explicitly specify the type of the provenance \sbol{prov:Activity} in more detail. If specified, it is RECOMMENDED that at least one \sbol{URI} of the \sbolmult{types:Activity}{type} property of an \sbol{prov:Activity} refers to a \sbol{URI} from \ref{tbl:activity_types}.

\subparagraph{The \sbolheading{startedAtTime} property}\label{sec:startedAtTime}
The \sbol{prov:startedAtTime} property is OPTIONAL and contains a DateTime (see section \ref{sec:DateTime}) value, indicating when the activity started.  If this property is present, then the \sbol{endedAtTime} property is REQUIRED.

\subparagraph{The \sbolheading{endedAtTime} property}\label{sec:endedAtTime}
The \sbol{prov:endedAtTime} property is OPTIONAL and contains a DateTime (see section \ref{sec:DateTime}) value, indicating when the activity ended.

\subparagraph{The \sbolheading{qualifiedAssociation} property}\label{sec:associations}
The \sbol{prov:qualifiedAssociation} property is OPTIONAL and MAY contain a set of \sbol{URI}s that refers to \sbol{prov:Association} objects.

\subparagraph{The \sbolheading{qualifiedUsage} property}\label{sec:usages}
The \sbol{prov:qualifiedUsage} property is OPTIONAL and MAY contain a set of \sbol{URI}s that refers to \sbol{Usage} objects.

\subparagraph{The \sbolheading{wasInformedBy} property}\label{sec:wasInformedBys}
The \sbol{prov:wasInformedBy} property is OPTIONAL and MAY contain a set of \sbol{URI}s that refers to other \sbol{prov:Activity} objects.

\subsubsection{Usage}
\label{sec:Usage}
How different entities are used in an \sbol{prov:Activity} is specified with the \sbol{prov:Usage} class, which is linked from an \sbol{prov:Activity} through the \sbol{prov:Usage} relationship. A \sbol{prov:Usage} is then linked to an \texttt{Entity} through the \texttt{Entity}'s \sbol{URI} and the role of this entity is qualified with the \sbolmult{roles:U}{prov:hadRole} property. When the \sbol{prov:wasDerivedFrom} property is used together with the full provenance described here, the entity pointed at by the \sbol{prov:wasDerivedFrom} property MUST be included in a \sbol{prov:Usage}.

\subparagraph{The \sbolheading{entity} property}\label{sec:entity}
The \sbol{prov:entity} property is REQUIRED and MUST contain a \sbol{URI} which MAY refer to an SBOL Identified object.

\subparagraph{The \sbolheading{hadRole} property}\label{sec:roles:U}
The \sbolmult{roles:U}{prov:hadRole} property is an OPTIONAL set of \sbol{URI}s that refer to particular term(s) describing the usage of an \sbol{prov:entity} referenced by the \sbol{prov:entity} property. Recommended terms that are defined in \ref{tbl:activity_types} can be used to indicate how the referenced \sbol{prov:entity} is being used in this \sbol{prov:Activity}.

\subsubsection{Association}
\label{sec:Association}
An \sbol{prov:Association} is linked to an \sbol{prov:Agent} through the \sbol{prov:agent} relationship. The \sbol{prov:Association} includes the \sbolmult{roles:A}{prov:hadRole} property to qualify the role of the \sbol{prov:Agent} in the \sbol{prov:Activity}.

\subparagraph{The \sbolheading{agent} property}\label{sec:agent}
The \sbol{prov:agent} property is REQUIRED and MUST contain a \sbol{URI} that refers to an \sbol{prov:Agent} object.

\subparagraph{The \sbolheading{hadRole} property}\label{sec:roles:A}
The \sbolmult{roles:A}{prov:hadRole} property is an OPTIONAL set of \sbol{URI}s that refers to particular term(s) that describes the the role of the \sbol{prov:agent} in the parent \sbol{prov:Activity}. 

\subparagraph{The \sbolheading{hadPlan} property}\label{sec:plan}
The \sbol{prov:hadPlan} property is OPTIONAL and contains a \sbol{URI} that refers to a \sbol{prov:Plan}.

\subsubsection{Plan}
\label{sec:prov:Plan}

 The \sbol{prov:Plan} entity can be used as a place holder to describe the steps (for example scripts or lab protocols) taken when an \sbol{prov:Agent} is used in a particular \sbol{prov:Activity}. 
...
\subparagraph{Example - Codon optimization}
 ...
 Codon optimization is a practical real-wold example where provenance properties can be applied. Using the current specification, the relationship between an original CDS and the codon-optimized version could simply be represented using the \sbol{prov:wasDerivedFrom} predicate, in a light-weight form. With more comprehensive use of the PROV ontology, the codon optimization can be represented as an \sbol{prov:Activity}. This \sbol{prov:Activity} can then include additional information, such as the \sbol{prov:Agent} responsible (in this case, codon-optimizing software), and additional parameters.
