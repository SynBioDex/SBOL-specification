...
The \sbol{subject} property is REQUIRED and MUST contain a \sbol{URI} that refers to a \sbol{ComponentInstance} or a \sbol{SequenceFeature} contained by the same parent \sbol{Component} that contains the \sbol{Constraint}.

\paragraph{The \sbolheading{object} property}\label{sec:object}
The \sbol{object} property is REQUIRED and MUST contain a \sbol{URI} that refers to a \sbol{ComponentInstance} or a \sbol{SequenceFeature} contained by the same parent \sbol{Component} that contains the \sbol{Constraint}. This \sbol{ComponentInstance} or \sbol{SequenceFeature} MUST NOT be the same \sbol{ComponentInstance} or \sbol{SequenceFeature} that the \sbol{Constraint} refers to via its \sbol{subject} property.
