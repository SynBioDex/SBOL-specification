...
The forms of these names follow the usual rules of English grammar. For example, sequenceFeature is the singular form of \sbol{sequenceFeatures}.
...
\begin{itemize}
 ...
 \item A \sbol{TopLevel} \sbol{identity} has the following pattern:
  ...
  \item A \sbol{TopLevel} object's \sbol{identity} property cannot be included as prefix for other \sbol{TopLevel} objects. E.g. The \path{BBa_J23070_seq} \sbol{Sequence} object cannot have the \path{https://synbiohub.org/public/igem/BBa_J23070/BBa_J23070_seq} \sbol{identity}, since the \path{https://synbiohub.org/public/igem/BBa_J23070} prefix is already used as an \sbol{identity} for the \path{BBa_J23070} \sbol{ComponentDefinition} object.
  \item The \sbol{identity} property of any child or nested object has the following pattern:\texttt{[parent]/[displayId]}, where \texttt{parent} is the \sbol{identity} of a parent object. Multiple layers of child objects are allowed using the same \texttt{[parent]/[displayId]} pattern recursively. E.g. The \texttt{annotation1} \sbol{SequenceAnnotation} object of the \path{BBa_J23070} \sbol{Component} entity will have the \path{https://synbiohub.org/public/igem/BBa_J23070/annotation1} \sbol{identity}. Similarly, the \texttt{loc1} \sbol{Location} of the \texttt{annotation1} \sbol{SequenceAnnotation} object will have the \path{https://synbiohub.org/public/igem/BBa_J23070/annotation1/loc1} \sbol{identity}.
  \end{itemize}
...
An SBOL object MUST NOT refer to itself via its own \sbol{wasDerivedFroms} property or form a cyclical chain of references via its \sbol{wasDerivedFroms} property and those of other SBOL objects. For example, the reference chain ``$A$ was derived from $B$ and $B$ was derived from $A$'' is cyclical.
...
In addition, this class has properties for describing and organizing the substructure of said design entity, including \sbol{Feature}, \sbol{feature}, and \sbol{constraint}.
...
The \sbol{subcomponents} property is OPTIONAL and MAY specify a set of \sbol{Feature} objects that are contained by the \sbol{Component}. The set of relations between \sbol{Feature} and \sbol{Component} objects is strictly acyclic (see \ref{sec:SubComponent}).
...
The \sbol{subcomponents} properties of \sbol{Component} objects can be used to construct a hierarchy of \sbol{Feature} and \sbol{Component} objects. 
...
This mapping is also subject to any restrictions on the positions of the \sbol{Feature} objects in the hierarchy that are imposed by the \sbol{SequenceFeature} or \sbol{Constraint} objects contained by the \sbol{Component} objects in the hierarchy.

A DNA \sbol{Component}, for example, could refer to a \sbol{Sequence} with an \external{IUPAC DNA} \sbol{encoding} and an \sbol{elements} \external{String} of ``{\tt gattaca}.'' In turn, this \sbol{Component} could contain a \sbol{Feature} that refers to a ``lower level''  \sbol{Component} that also refers to a \sbol{Sequence} with an \external{IUPAC DNA} \sbol{encoding}. Consequently, a consistent \sbol{elements} \external{String} of this ``lower level'' \sbol{Sequence} could be ``{\tt gatta},'' or perhaps ``{\tt tgta}'' if the \sbol{Feature} is positioned by a \sbol{SequenceFeature} that contains a \sbol{Location}  with an \sbol{orientation} of ``reverse complement'' (see \ref{sec:Location}).
\todo{Update this in SEP 41}

%%%%MODULE
The \sbol{modules} property is OPTIONAL and MAY specify a set of \sbol{Feature} objects contained by the \sbol{Component}.
Note that the set of relations between \sbol{Feature} and \sbol{Component} objects is strictly acyclic. \todo{update this in SEP 41}
...
In addition, each \sbol{SequenceFeature} can  position a \sbol{Feature} of the \sbol{Component} at the region specified by its \sbol{Location} objects (see \ref{sec:Location}). 
...
\paragraph{The \sbolheading{role} property}\label{sec:roles:C}
...
\paragraph{The \sbolheading{sourceLocation} property}\label{sec:sourceLocation}

The \sbol{sourceLocation} property is an OPTIONAL set of zero or more \sbol{Location} objects that indicate which \sbol{elements} of a \sbol{ComponentDefinition}'s \sbol{Sequence} are to be included in the \sbol{Component}'s parent. The \sbol{sourceLocation} property
...
\paragraph{The \sbolheading{inChildOf} property}\label{sec:inChildOf}
...
\paragraph{The \sbolheading{feature} property}\label{sec:feature:CR}
...
\paragraph{The \sbolheading{type} property}\label{sec:type:ER}
...
The \sbol{ExternalReference} class has been introduced so that external definitions in databases like CHEBI or UniProt can be referenced.

\paragraph{The \sbolheading{type} property}\label{sec:type:ER}

The \sbolmult{type:ER}{type} property is REQUIRED and contains one or more \sbol{URI}s. The \sbolmult{type:ER}{type} property is identical to its use in \sbol{Component}

\paragraph{The \sbolheading{definition} property}\label{sec:definition:ER}
...
When \sbol{Component} objects are composed into structural and functional hierarchies using \sbol{Feature} objects, it is often the case that some \sbol{Feature} objects are intended to represent the same entity in the overall design. The purpose of the \sbol{MapsTo} class is to make these identity relationships clear and explicit.  For example, consider a \sbol{Component} for a genetic inverter that includes a \sbol{Feature} for an abstract repressor protein.  When this \sbol{Component} is instantiated within a ``higher level'' \sbol{Component} that includes a \sbol{Feature} for a LacI protein, the \sbol{MapsTo} object can be used to indicate that the repressor protein in the first \sbol{Component} is LacI in the context of the composite design.

In particular, a \sbol{MapsTo} object provides two pieces of information:
\begin{itemize}
\item An identity relationship between two \sbol{Feature} objects, the first contained by the ``lower level'' definition of the \sbol{Feature} that owns the
  \sbol{MapsTo}, and the second contained by the ``higher level'' definition that contains the \sbol{Feature} that owns the \sbol{MapsTo}. The \sbol{remote} property of a \sbol{MapsTo} refers to the first ``lower level'' \sbol{Feature}, while the \sbol{local} property refers to the second ``higher level'' \sbol{Feature}.
\item Instructions on how to interpret \sbol{local} and \sbol{remote} \sbol{Feature} objects that refer to different \sbol{Component} objects (that is, non-identical objects). These are specified using the \sbol{refinement} property of the \sbol{MapsTo} class.
\end{itemize}

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.95]{images/MapsTo_Diagram3}
\caption{Linking \sbol{Feature} objects using \sbol{MapsTo} entities. Boxes with diagrams represent \sbol{Component} objects, boxes with the C label represent \sbol{Feature} objects, and boxes with the M label represent \sbol{MapsTo} objects. In both diagrams, a promoter-RBS \sbol{Component} and a RBS-CDS \sbol{Component} are being composed to form the \sbol{Component} of a complete transcriptional unit. In the left-hand diagram, the two \sbol{Feature} objects inside the promoter-RBS \sbol{Component} and RBS-CDS \sbol{Component} objects both refer to an abstract RBS \sbol{Component} that lacks a sequence (white semicircle). Through the use of \sbol{MapsTo} objects with \sbol{refinement} set to useLocal, these ``lower level'' \sbol{Component} objects are effectively overridden by that of the green RBS in the \sbol{Component} of the complete transcriptional unit. In the right-hand diagram, however, the two ``lower level'' RBS \sbol{Component} objects do not lack sequences and it is the ``higher level'' RBS \sbol{Component} that is abstract. In this case, one of the \sbol{MapsTo} objects has a useRemote \sbol{refinement}, resulting in the green RBS \sbol{Component} overriding that of the abstract RBS in the ``higher level'' \sbol{Component}.}
...
In both examples, the two ``lower level'' \sbol{Component} objects each contain a RBS \sbol{Feature} that is intended to represent the same design entity in the ``higher level'' \sbol{Component} of the transcriptional unit.

In order to explicitly represent the identity relationships in this example, a new RBS \sbol{Feature} needs to be created inside the ``higher level'' \sbol{Component}.
This ``higher level'' \sbol{Feature} then needs to be linked to the equivalent ``lower level'' \sbol{Feature} objects by means of the \sbol{MapsTo} class, using one \sbol{MapsTo} object per link.
For example, in order to link the ``higher level'' RBS \sbol{Feature} to the ``lower level'' RBS \sbol{Feature} of the promoter-RBS \sbol{Component}, a \sbol{MapsTo} has to be created on the ``higher level'' promoter-RBS \sbol{Feature}. The \sbol{local} property of this \sbol{MapsTo} then has to refer to the ``higher level'' RBS \sbol{Feature}, while its \sbol{remote} property has to refer to the ``lower level'' RBS \sbol{Feature}.
In this way, many ``lower level'' \sbol{Feature} objects can be linked together at the ``higher level'' using as an equal number of \sbol{MapsTo} objects, each one referring to a different \sbol{remote} \sbol{Feature}, but all referring to the same \sbol{local} \sbol{Feature}.
...
d to refer to the \sbol{Feature} contained by the ``higher level'' \sbol{Component} or \sbol{Component}. This \sbol{local} \sbol{Feature} MUST be contained by the \sbol{Component} or \sbol{Component} that contains the \sbol{Feature} or \sbol{Feature} that owns the \sbol{MapsTo}.

\paragraph{The \sbolheading{remote} property}\label{sec:remote}
This REQUIRED property has a data type of \sbol{URI} and is used to refer to the \sbol{Feature} contained by the ``lower level'' \sbol{Component}.
This \sbol{remote} \sbol{Feature} MUST be contained by the \sbol{Component} or \sbol{Component} that is the \sbolmult{definition:CI}{definition} of the \sbol{Feature} or \sbol{Feature} that owns the \sbol{MapsTo}.

\paragraph{The \sbolheading{refinement} property}\label{sec:refinement}
The \sbol{refinement} property is REQUIRED and has a data type of \sbol{URI}. Each \sbol{MapsTo} object MUST specify the relationship between its \sbol{local} and \sbol{remote} \sbol{Feature} objects using one of the REQUIRED \sbol{refinement} \sbol{URI}s provided in \ref{tbl:mapsto_refinement}.
Note that if multiple \sbol{MapsTo}s belonging to the \sbol{Feature}s of a \sbol{Component} have \sbol{local} properties that refer to the same \sbol{Feature}, then there MUST NOT be more than one such \sbol{MapsTo} that has a \sbol{refinement} property that contains the \sbol{URI} \url{http://sbols.org/v2\#useRemote}. Similarly, if multiple \sbol{MapsTo}s belonging the \sbol{Feature}s and \sbol{Feature}s of a \sbol{Component} have \sbol{local} properties that refer to the same \sbol{Feature}, then there MUST NOT be more than one such \sbol{MapsTo} that has a \sbol{refinement} property that contains the \sbol{URI} \url{http://sbols.org/v2\#useRemote}.
...
\sbol{GenericLocation} class is included as a starting point for specifying regions on \sbol{Sequence} objects with different \sbol{encoding} properties and potentially nonlinear structure. This class can also be used to set the \sbol{orientation} of a \sbol{SequenceFeature} and any associated \sbol{Feature} when their parent \sbol{Component} is a partial design that lacks a \sbol{Sequence}.
...
Consequently, its \sbol{subcomponents} property SHOULD contain one or more \sbol{Feature} objects that describe its substructure (referred to hereafter as template \sbol{Feature} objects), and its \sbol{constraint} property MAY also contain one or more \sbol{Constraint} objects that constrain this substructure.
...
If the \sbol{variable} property of one of these \sbol{VariableComponent} objects refers to a template \sbol{Feature}, then the \sbol{subcomponents} property of the derived \sbol{Component} SHOULD contain as many \sbol{Feature} objects derived from the template \sbol{Feature} as specified by the \sbol{operator} property of the \sbol{VariableComponent} (see \ref{tbl:operator}). In addition, the \sbol{instanceOf} properties of these derived \sbol{Feature} objects MUST refer to \sbol{Component} objects specified by the \sbol{variants}, \sbol{variantCollections}, or \sbol{variantDerivations} property of the \sbol{VariableComponent}.

If no \sbol{variable} property of one of these \sbol{VariableComponent} objects refers to a template \sbol{Feature}, then the \sbol{subcomponents} property of the derived \sbol{Component} SHOULD contain exactly one \sbol{Feature} with a \sbol{wasDerivedFroms} property that refers to the template \sbol{Feature}. The \sbol{instanceOf} property of this derived \sbol{Feature} MUST refer to the \sbol{Component} referred to by the \sbol{instanceOf} property of the template \sbol{Feature}.
...
The \sbol{variable} property is REQUIRED and MUST contain a URI that refers to a template \sbol{Feature} in the template \sbol{Component}. If the \sbol{wasDerivedFroms} property of a \sbol{Feature} refers to this template \sbol{Feature}, then the \sbol{instanceOf} property of the derived \sbol{Feature} MUST refer to either (1) a \sbol{Component} referred to by the \sbol{variants} property of the \sbol{VariableComponent}, (2) a \sbol{Component} from a \sbol{Collection} referred to by the \sbol{variantCollections} property of the \sbol{VariableComponent}, or (3) a \sbol{Component} derived from a \sbol{CombinatorialDerivation} referred to by the \sbol{variantDerivations} property of the \sbol{VariableComponent}.

If the \sbolmult{roles:CD}{roles} property of the template \sbol{Feature} contains one or more URIs, then the \sbolmult{roles:CD}{roles} property of the derived \sbol{Feature} SHOULD also contain those URIs.
...
\sbol{Feature} from the template \sbol{Feature}.
...
\begin{table}[ht]
  \begin{edtable}{tabular}{lp{4in}}
    ...
    \midrule
    \url{http://sbols.org/v2#zeroOrOne} & No more than one \sbol{Feature} in the derived \sbol{Component} SHOULD have a \sbol{wasDerivedFroms} property that refers to the template \sbol{Feature}. \\
        \url{http://sbols.org/v2#one} & Exactly one \sbol{Feature} in the derived \sbol{Component} SHOULD have a \sbol{wasDerivedFroms} property that refers to the template \sbol{Feature}. \\
\url{http://sbols.org/v2#zeroOrMore} & Any number of \sbol{Feature} objects in the derived \sbol{Component} MAY have \sbol{wasDerivedFroms} properties that refer to the template \sbol{Feature}. \\
\url{http://sbols.org/v2#oneOrMore} & At least one \sbol{Feature} in the derived \sbol{Component} SHOULD have a \sbol{wasDerivedFroms} property that refers to the template \sbol{Feature}. \\
    \bottomrule
  \end{edtable}
...
\item Each SBOL 1.1 \external{SequenceFeature} also maps to an SBOL 2.x \sbol{Feature}, which represents the instantiation or usage of the appropriate \sbol{Component}.
