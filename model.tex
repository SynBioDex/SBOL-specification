...
The \sbol{location} property is OPTIONAL set which points to a \sbol{Location} on the parent \sbol{Component} sequence; if \sbol{location} is missing, this indicates a part / sub-part relationship for which sequence details have not (yet) been determined.

The \sbol{Location} record(s) specified by a \sbol{SubComponent} are subject to the same restrictions currently in place for SequenceAnnotation Location. Concretely, two \sbol{Location} records attached to the same \sbol{SubComponent} MUST NOT overlap in their range as it would not be clear what that means. The \sbol{Location} of two separate \sbol{SubComponents} may overlap.
...
\end{center}
\end{figure} \todo{change figure for SequenceFeature instead of SequenceAnnotation}
