...
\lstdefinelanguage{sbol}
    {morekeywords={xmlns:sbol,rdf:about,sbol:displayId,sbol:persistentIdentity,sbol:version,sbol:timeStamp,sbol:name,sbol:description,sbol:member,sbol:Collection },
...
An optional, human readable name property, with a data type of \texttt{String}. If a \text{Documented} object lacks a \texttt{displayId}, it is expected that software tools will instead display the entity's texttt{name} or texttt{identity} as a last resort.
