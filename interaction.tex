...
An \sbol{Interaction} MAY have one or more \sbol{hasParticipation} properties, each of type \sbol{URI} that MUST reference a \sbol{Participation} object (see \ref{sec:Participation}), each of which identifies the \sbolmult{role:P}{role} that its referenced \sbol{Feature} plays in the \sbol{Interaction}.
