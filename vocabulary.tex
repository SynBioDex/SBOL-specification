...
Note that, because identities are \sbol{URI}s, both \sbol{URI}s and URI prefixes MUST be compared under the rules for \sbol{URI} equality. This means, for example, that namespaces \path{https://synbiohub.org/}, \path{https://synbiohub.org}, and \path{https://synbiohub.org:443/} are all equal.

Whenever a \sbol{TopLevel} object's \sbol{URI} is a URL (e.g., following the conventions of HTTP(S) rather than a UUID), its structure MUST comply with the following rules:
