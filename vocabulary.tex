...
\threezeroone{\subsection{SBOL Types}
  \label{sec:sbolTypes}

All SBOL objects are given the most specific \external{rdfType} in the SBOL 3 namespace (\uri{http://sbols.org/v3\#}) that defines the type of the object.  Namely, an object MUST have no more than one \external{rdfType} property in the \uri{http://sbols.org/v3\#} namespace.

For example, an object cannot have both the \external{rdfType} of \sbol{Collection} and \sbol{Component}.  Also, a \sbol{SubComponent} would have this \external{rdfType} and not also include \external{rdfType}s for classes that it inherits from, such as \sbol{Feature} and \sbol{Identified}.
}
