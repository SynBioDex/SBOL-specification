...
\sbol{Sequence}, and threezeroone{\st{VariableComponent}} \sbol{VariableFeature} classes are omitted or abstracted, since they are essentially unchanged in SBOL 3.x except for the following minor changes:
\threezeroone{\begin{itemize}
\item In \sbol{Sequence}, the \sbol{encoding} property values for IUPAC map according to  \ref{tbl:sequence_encoding_mapping}. 
\item The SBOL 2.x \sbol{VariableComponent} class has been renamed \sbol{VariableFeature}.
\item In \sbol{VariableComponent}, the SBOL 2.x \external{operator} property maps to the SBOL 3.x \sbol{cardinality} property.
\item In \sbol{VariableComponent}, the \sbol{variantMeasure} property has been added, which does not exist in SBOL 2.x.
