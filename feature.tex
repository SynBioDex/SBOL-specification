...
It is RECOMMENDED that these \sbolmult{role:F}{role} property \sbol{URI}s identify terms that are compatible with the \sbolmult{type:CD}{type} properties of the \sbol{Feature}'s parent \sbol{Component}.
...
Likewise, for any feature that is a \sbol{SubComponent}, the \sbolmult{role:F}{role} SHOULD be compatible with the \sbolmult{type:CD}{type} of the \sbol{Component} that it links to through its \sbol{instanceOf} property.

\subparagraph{The \sbolheading{orientation} property}
\label{sec:orientation:F}
The \sbolmult{orientation:F}{orientation} property is OPTIONAL and has a data type of \sbol{URI}. This can be used to indicate how any associated double-stranded \sbol{Feature} is oriented on the \sbol{elements} of a \sbol{Sequence} from their parent \sbol{Component}. \ref{tbl:orientation_types} provides a list of REQUIRED \sbol{orientation:F} \sbol{URI}s. If a \sbol{Feature} object has an \sbol{orientation:F}, then it MUST come from \ref{tbl:orientation_types}.
...
If zero \sbol{SubComponent} \sbolmult{role:F}{role} properties are given and no \sbol{SubComponent} \sbol{roleIntegration} is given, then \url{http://sbols.org/v2\#mergeRoles} is assumed.
It is RECOMMENDED to specify \sbol{SubComponent} \sbolmult{role:F}{role} values only if the result would differ from the set of \sbolmult{role:CD}{role} values belonging to this \sbol{SubComponent}'s included \sbol{Component}.
...
\begin{table}[ht]
  \begin{edtable}{tabular}{lp{4in}}
    ...
    \bottomrule
  \end{edtable}
  \caption{Each \sbol{roleIntegration} mode is associated with a rule governing how a \sbol{SubComponent}'s \sbolmult{role:F}{role} values are to be combined with the included \sbol{Component}'s \sbolmult{role:CD}{role} values.}
...
Among other things, as described in the previous section, this \sbol{Component} effectively provides information about the \sbolmult{type:CD}{type} and \sbolmult{role:CD}{role} of the \sbol{SubComponent}.
...
If no \sbolmult{hasLocation:SC}{hasLocation} is defined, this indicates a part / sub-part relationship for which sequence details have not (yet) been determined or involving types for which sequence relationships are not relevant (e.g., inclusion of a reaction sequence within a metabolic network).
...
The \sbol{inChildOf} property must refer to a \sbol{SubComponent} pointed directly to by the parent of the \sbol{ComponentReference} (i.e., as a \sbol{SubComponent} of \sbol{Component}, or an \sbol{inChildOf} of \sbol{ComponentReference}). 
...
If the \sbol{Feature} is a \sbol{ComponentReference}, then that \sbol{ComponentReference} MUST BE a child of the \sbol{ComponentReference} linking to it via \sbolmult{hasFeature:CR}{hasFeature}, and it acts as a hierarchical link in a chain of references.
Otherwise, if the \sbol{hasFeature} refers to any other type of \sbol{Feature}, that feature MUST BE a child of the \sbol{SubComponent} pointed to by \sbol{inChildOf}.
