...
\caption[]{Composite \sbol{Component} objects for the LacI inverter. In the case of the \sbol{Component} that represents the TetR gene, its sub-\sbol{ComponentInstance} objects are located as \sbol{Range}s along its \sbol{Sequence} using \sbol{SequenceAnnotation} objects. The \sbol{Component} that represents the IPTG-LacI complex, however, has no \sbol{Sequence} and its sub-\sbol{ComponentInstance} objects are composed without any data about their relative positions.}
...
\caption[]{Composite \sbol{Component} of the LacI/TetR toggle switch. This \sbol{Component} contains the \sbol{ComponentInstance} objects that instantiate LacI and TetR inverter \sbol{Component} objects. It also contains \sbol{ComponentInstance} objects that instantiate the \sbol{Component} objects for the LacI/TetR transcription factors and IPTG/aTc small molecules. These \sbol{ComponentInstance} objects each participate in a non-covalent binding \sbol{Interaction}. To complete the composition of the toggle switch, \sbol{MapsTo} objects are used to indicate that the output of the LacI inverter \sbol{Component} is identical to the input of the TetR inverter \sbol{Component} and vice versa.
}
...
% Each \sbol{Component} also contains the \sbol{ComponentInstance}s that participate in \sbol{Interaction}s and are defined by the same \sbol{Component}s as the parallel \sbol{ComponentInstance}s in the structural hierarchy of the toggle switch. Finally, \sbol{MapsTo} entities are used to refine which \sbol{ComponentInstance}s of the functional hierarchy are identical or map them to \sbol{ComponentInstance}s in the structural hierarchy.
