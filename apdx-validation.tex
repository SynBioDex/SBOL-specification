...
%% CHRIS: should these be best practice or uncheckable required
\printModeling{\sbol{TopLevel} objects with a \sbol{wasGeneratedBys} property that includes a reference to an \sbol{Activity} with a child \sbol{Association} that has a \sbolmult{roles:A}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#design} SHOULD be a \sbol{ModuleDefinition}, \sbol{ComponentDefinition}, or \sbol{Sequence}.\\ 
Reference: \sec{sec:provenance}}

\printModeling{\sbol{TopLevel} objects with a \sbol{wasGeneratedBys} property that includes a reference to an \sbol{Activity} with a child \sbol{Association} that has a \sbolmult{roles:A}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#build} SHOULD be an \sbol{Implementation}.\\ 
Reference: \sec{sec:provenance}}
...
\printModeling{\sbol{TopLevel} objects with a \sbol{wasGeneratedBys} property that includes a references to an \sbol{Activity} with a child \sbol{Association} that has a \sbolmult{roles:A}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#learn} SHOULD be a \sbol{Model}, a \sbol{ModuleDefinition}, a \sbol{ComponentDefinition}, a \sbol{Sequence}, an \sbol{Attachment}, or a \sbol{Collection} of \sbol{Attachment} objects.\\ 
Reference: \sec{sec:provenance}}
}
...
\printValid{The \sbol{startedAtTime} property of an \sbol{Activity} object is OPTIONAL and MAY contain a \sbol{DateTime}.  \\ Reference: \sec{sec:Activity}}

\printValid{The \sbol{endedAtTime} property of an \sbol{Activity} object is OPTIONAL and MAY contain a \sbol{DateTime}.  \\ Reference: \sec{sec:Activity}}

\printValid{The \sbol{associations} property of an \sbol{Activity} is OPTIONAL and MAY contain a set of \sbol{Association} objects.\\ Reference: \sec{sec:Activity}}
...
\printComplete{Each \sbol{URI} contained by the \sbol{wasInformedBys} property of an \sbol{Activity} MUST refer to a \sbol{Activity} object.\\ Reference: \sec{sec:Activity}}

%% CHRIS: Not sure about these
\printModeling{An \sbol{Activity} with a child \sbol{Association} that has a \sbolmult{roles:A}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#design} SHOULD NOT have child \sbol{Usage} objects that have \sbolmult{roles:U}{roles} properties that contain the \sbol{URI}s~\url{http://sbols.org/v2\#build} or~\url{http://sbols.org/v2\#test}.\\ Reference: \sec{sec:Activity}}
...
%% CHRIS: Not sure about these
\printModeling{A \sbol{Usage} that has a \sbolmult{roles:U}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#design} SHOULD refer to a \sbol{ModuleDefinition}, \sbol{ComponentDefinition}, or \sbol{Sequence}.\\ Reference: \sec{sec:Usage}}

\printModeling{A \sbol{Usage} that has a \sbolmult{roles:U}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#build} SHOULD refer to a \sbol{Implementation}.\\ Reference: \sec{sec:Usage}}
...
\printModeling{A \sbol{Usage} that has a \sbolmult{roles:U}{roles} property that contains the \sbol{URI}~\url{http://sbols.org/v2\#learn} SHOULD refer to a \sbol{Model}, a \sbol{ModuleDefinition}, a \sbol{ComponentDefinition}, a \sbol{Sequence}, an \sbol{Attachment}, or a \sbol{Collection} of \sbol{Attachment} objects.\\ Reference: \sec{sec:Usage}}
...
\printValid{An \sbol{Plan} MUST NOT have properties other than the following: \sbol{identity},\\ \sbol{persistentIdentity}, \sbol{displayId}, \sbol{version}, \sbol{wasDerivedFroms}, \sbol{wasGeneratedBys}, \sbol{name},\\ \sbol{description}, and \sbol{annotations}.\\ Reference: \sec{sec:Plan}}
}
...
%% CHRIS: why is this best practice and not required?
...
%% CHRIS: not sure we need this, since we treat these as sets
...
\sbol{wasDerivedFroms} property that refers to the \sbol{variable} \sbol{Component} of the \sbol{VariableComponent}.
\\ Reference: \sec{sec:VariableComponent}}
