...
\Ctodo{Rule below needs to change if becomes a set of locations}

\printValid{The \sbol{location} is a required property for all \sbol{SequenceAnnotation} objects and is a \sbol{Location} class object. (Reference: \sec{sec:SequenceAnnotation})}
...
\Ctodo{Written assuming change in Location class to remove MultiRange}
...
\Ctodo{required local/remote URIs}

\Ctodo{Maps to must not reference private acesss components}
...
\printModeling{A least one type in the set of \sbol{types} should be a URI from the Systems Biology Ontology (SBO). (Reference: \sec{sec:Interaction})}
...
\Dtodo{Should this be required and changed to "types"?}
