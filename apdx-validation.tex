...
\printValid{An SBOL document MUST be serialized as a valid RDF/XML file.}
...
\printValid{The only properties a \sbol{Sequence} may have are: \sbol{identity}, \sbol{persistentIdentity}, \sbol{displayId}, \sbol{version}, \sbol{wasDerivedFrom}, \sbol{name}, \sbol{description}, \sbol{annotations}, \sbol{elements}, and \sbol{encoding}.
...
\todo{split into human / best-practice}
...
\todo{currently thrown if more than one from from SBO}
...
\todo{currently thrown if more than one from SBO}
...
%% TODO: Goksel will check his code
...
\todo{update as above}
\printValid{A \sbol{GenericTopLevel} object MUST inherit all properties of the \sbol{TopLevel} class.\\ Reference: \sec{sec:GenericTopLevel}}
